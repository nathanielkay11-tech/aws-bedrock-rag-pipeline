...
**Upload flow:** Lawyer fills in uploader name, matter ID, 
document type, and selects a file. Frontend calls POST /upload 
...
## ADR-014: S3 Folder Path Carries Upload Metadata; Pre-Signed URL Pattern for Browser Upload
**Date:** 06 May 2026
**Decision:** Two related decisions: uploader name and matter ID 
are encoded in the S3 folder path; browser uploads use a 
pre-signed URL generated by a dedicated presign Lambda.
**Reason — folder path:** The S3 event delivered to the 
ingestion Lambda contains only the object key. Encoding 
uploader name and matter ID in the path is the minimal change 
that makes this information available to the ingestion Lambda 
without adding metadata tag infrastructure.
**Path structure:**
`matters/<matter-id>/<document-type>/<uploader-name>/<filename>`
**Reason — pre-signed URL:** Browser security restrictions 
prevent direct S3 uploads without exposing AWS credentials 
to the client. The presign Lambda generates a time-limited 
pre-signed PUT URL server-side and returns it to the frontend. 
The browser uploads directly to S3 using that URL — 
credentials never leave the server.
**Phase 2:** Uploader name extracted from Cognito JWT token 
automatically. Matter ID selected from permitted matter list. 
Pre-signed URL pattern retained — only the source of the 
metadata values changes.
**Outcome:** Secure browser-to-S3 upload with full metadata 
encoded in the path. Ingestion Lambda authentication-agnostic 
by design.
...
## ADR-017: Pre-Signed URL Lambda for S3 Upload
**Date:** 06 May 2026
**Decision:** Implement a dedicated pre-signed URL Lambda to 
handle document uploads from the frontend to S3.
**Reason:** Browser security restrictions prevent direct S3 
uploads without a pre-signed URL. The frontend calls the 
presign Lambda via API Gateway to obtain a time-limited 
pre-signed PUT URL, then uploads directly to S3 using that 
URL. This keeps AWS credentials server-side and never exposed 
to the browser.
**Architecture:** Frontend → POST /upload → Presign Lambda → 
returns pre-signed URL → Frontend PUTs file directly to S3 → 
S3 triggers Ingestion Lambda automatically.
**Note:** This component was identified as missing during 
initial deployment testing and added as a corrective measure.
**Outcome:** Secure browser-to-S3 upload with credentials 
